"""Connection lifecycle for Dashlane Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from dashlane_client import DashlaneClient
...
@chat.function(
    "connect_dashlane",
    "Connect Dashlane account via credentials.",
    action_type="write",
    chain_callable=True,
    event="dashlane-connector.connect_dashlane",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_dashlane(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Dashlane Connector."""
    client = DashlaneClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Dashlane Account",
...
@chat.function(
    "list_connections",
    "List connected Dashlane accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_dashlane",
    "Disconnect Dashlane account.",
    action_type="write",
    chain_callable=True,
    event="dashlane-connector.disconnect_dashlane",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_dashlane(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
